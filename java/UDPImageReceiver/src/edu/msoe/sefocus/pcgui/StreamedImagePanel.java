...
import edu.msoe.sefocus.core.CapturedImage;
...
public class StreamedImagePanel extends JPanel implements Runnable {
    ...
    private int linesPerUDPTransmission = 32;
    ...
    /**
     * This panel will be sued to help keep the diagnostic controls current.
     */
    ...
    public StreamedImagePanel(int port, int width, int height) throws IOException {
        ...
        this.linesPerUDPTransmission = 1;
        portNumber = port;
    ...
    public void run() {
        ...
        byte[] receptionBuffer = new byte[((this.imageWidth * 3 + 24) * linesPerUDPTransmission)];
        ...
        while (keepGoing) {
            ...
            if ((receptionBuffer.length != ((this.imageWidth * 3 + 24) * linesPerUDPTransmission)) || (lineRGBDataBuffer.length != this.imageWidth * 3)) {
                receptionBuffer = new byte[((this.imageWidth * 3 + 24) * linesPerUDPTransmission)];
            ...
            try {
                ...
                // Read how many lines are contained in each UDP datagram.
                int tempLinesPerUDPDatagram = dis.readInt();

                if (tempLinesPerUDPDatagram<0 || tempLinesPerUDPDatagram > 100)
                {
                    // Ignore it as it is temporarily out of bounds.
                    System.out.println(tempLinesPerUDPDatagram);
                }
                else if (tempLinesPerUDPDatagram != linesPerUDPTransmission) {
                    linesPerUDPTransmission = tempLinesPerUDPDatagram;
                } else {
                    for (int t = 0; t < linesPerUDPTransmission; t++) {
                        ...
                        if (diagPanel.isExtraDebugInfoActive()) {
                            ...
                            if (diagPanel.isPrintHexLogEnabled()) {
                                for (int temp = 0; temp < lineRGBDataBuffer.length; temp++) {
                                    System.out.printf("%x", lineRGBDataBuffer[temp]);
                                }
                            }
                            System.out.println();
                        }
